...
def save_figure(path):
    ...
    bk_ = matplotlib.backends.backend
    assert path != "", "The path is Empty!"

    if bk_ in ['TkAgg', 'Agg', 'GTKAgg']:
        path = path+".png"
    elif bk_ in ['PDF', 'pdf']:
        path = path+".pdf"
    elif bk_ in ['PS', 'ps']:
...
def corr_table(table, dvecs, table_name):
    ...
    np.savetxt(pkd+table_name, new, ("%1.3g", "%1.3g", "%1.3g", "%1.3g",
                                     "%1.3g", "%1.3g", "%1.3g", "%1.3g",
                                     "%1.3g", "%1.3g", "%1.3g"))
    return new
...
#Corr is /sqrt(self.H0)...
    ...
    def get_z(self, snap):
        """ Get redshift associated with a snapshot """
        ind = np.where(np.array(self.Snaps) == snap)
        assert np.size(ind), "{0} does not exist!".format(str(snap))
        return self.Zz[ind]
    ...
    def get_snap(self, redshift):
        ...
        ind = wheref(self.Zz, redshift)
        assert np.size(ind), "No snapshot at redshift "+str(redshift)
        return str(np.asarray(self.Snaps)[ind][0])

    def get_sdss_bins(self, redshift):
        """Get the k bins at a given redshift in h/Mpc units"""
        return self.sdsskbins*self.Hubble(redshift)/(1.0+redshift)
        #Corr is /sqrt(self.H0)...
    ...
    def plot_z(self, Knot, redshift, title="", ylabel="", legend=True):
        ...
        #Load best-fit
        (simk, bfkp) = self.loadpk(Knot.bstft+\
                   self.suf+self.pre+self.get_snap(redshift)+self.ext, self.bfbox)
        #Setup figure plot.
        ind = wheref(self.Zz, redshift)
        plt.figure(ind[0][0])
        plt.clf()
        ...
        plt.ylabel(ylabel)
        plt.xlabel(r"$k\; (w \mathrm{Mpc}^{-1})$")
        line = np.array([])
        legname = np.array([])
        for sim in Knot.names:
            (k, Pk) = self.loadpk(sim+self.suf+\
                                  self.pre+self.get_snap(redshift)+self.ext, self.box)
            oi = np.where(simk <= k[-1])
            ti = np.where(simk[oi] >= k[0])
            relP = rebin(Pk, k, simk[oi][ti])
            relP = relP/rebin(bfkp, simk, simk[oi][ti])
            #        relP = smooth_rebin(relP, simk)
            line = np.append(line, plt.semilogx(simk[oi][ti]/self.H0,
                                                relP, linestyle="-"))
    ...
    def plot_power(self, path, redshift, camb_filename="", colour="black"):

        """ Plot absolute power spectrum, not relative"""
        (k_g, Pk_g) = self.loadpk(path+self.suf+self.pre+\
                                  self.get_snap(redshift)+self.ext, self.box)
        plt.loglog(k_g, Pk_g, color=colour)
        plt.xlim(0.01, k_g[-1]*1.1)
        plt.ylabel("P(k) /(h-3 Mpc3)"+camb_filename)
...
#        relP = smooth_rebin(relP, onek)
    ...
    def compare_two(self, one, two, redshift):
        ...
        relP = relP/rebin(oneP, onek, onek[onei][twoi])
        onek = onek[onei][twoi]
        sdss = self.get_sdss_bins(redshift)
    ...
    def compare_two_table(self, onedir, twodir):
        ...
        for i in np.arange(0, nz):
            sim = self.pre+self.get_snap(self.Zz[i])+self.ext
            table[-1-i, :] = self.compare_two(onedir+sim, twodir+sim, self.Zz[i])
        return table


    def plot_compare_two_sdss(self, onedir, twodir,
                              zzz=np.array([]), out="", title="",
        ...
        for z in zzz:
            sim = self.pre+self.get_snap(z)+self.ext
        ...
        if ylabel != "":
            plt.ylabel(ylabel)
        if legend:
            plt.legend(line, legname, bbox_to_anchor=(0., 0, 1., .25),
                       loc=3, ncol=3, mode="expand", borderaxespad=0.)
        if ymax != 0 and ymin != 0:
            plt.ylim(ymin, ymax)
        plt.xlim(sdss[0], sdss[-1])
        plt.xticks(np.array([sdss[0], 3e-3, 5e-3, 0.01, sdss[-1]]),
    ...
    def plot_compare_two_all(self, onedir, twodir, zzz=np.array([]),
                             out="", title="", ylabel="", ymax=0, ymin=0,
                             colour="", legend=False):
        ...
        legname = np.array([])
        for z in zzz:
            line = np.append(line, self.plot_compare_two(
                onedir+self.pre+self.get_snap(z)+self.ext, self.bfbox,
                twodir+self.pre+self.get_snap(z)+self.ext, self.box, colour))
            legname = np.append(legname, "z="+str(z))
        if title == "":
    ...
    def GetFlat(self, dirr, si=0.0):
        ...
        for i in np.arange(0, np.size(self.Snaps)-1):
            ...
            (k, Pk) = self.loadpk(dirr+self.Snaps[i]+self.ext, self.box)
            ...
            for j in np.arange(1, np.size(sdss)):
                sicorr[j] = 1+a**2+2*a*(math.sin(2271*kmids[j+1])\
                                        -math.sin(2271*kmids[j]))/(kmids[j+1]-kmids[j])/2271
            sdss = self.get_sdss_bins(self.Zz[i])
            Pk_sdss[-i-1, :] = rebin(Pk, k, sdss)*scale*sicorr
        return Pk_sdss

    def GetDiff(self, dir1, dir2, onebox, twobox):

        """Gets the difference between two power spectra
        in arbitrary directories, with arbitrary box sizes"""

        diff = np.zeros([self.Zz.size, self.get_sdss_bins(3.0).size])
        i = 0
        for z in self.Zz:
            (onek, oneP) = self.loadpk(dir1+self.pre+self.get_snap(z)+self.ext, onebox)
            (twok, twoP) = self.loadpk(dir2+self.pre+self.get_snap(z)+self.ext, twobox)
            onei = np.where(onek <= twok[-1])
            twoi = np.where(onek[onei] >= twok[0])
            relP = rebin(twoP, twok, onek[onei][twoi])
            relP = relP/rebin(oneP, onek, onek[onei][twoi])
            onek = onek[onei][twoi]
#            relP = smooth_rebin(relP, onek, self.get_sdss_bins(z))
            i = i+1
#            diff[-i, :] = relP
#        diff = np.transpose(diff)
    ...
    def calc_z(self, redshift, s_KNOT, kbins):
        ...
        for i in np.arange(0, np.size(s_KNOT.names)):
            (k, PowerFluxes[i, :]) = self.loadpk(s_KNOT.names[i]+\
                                                 self.suf+snap+self.ext, s_KNOT.bfbox)
    ...
    def flux_deriv(self, PFdif, pdif, qdif=np.array([])):
        ...
        if np.size(pdif) != np.size(PFdif):
            raise DataError(str(np.size(pdif))+\
                            " parameter values, but "+str(np.size(PFdif))+" P_F values")
        if np.size(pdif) < 2:
    ...
    def Get_Error_z(self, Sim, bstft, box,
                    derivs, params, redshift, qarams=np.empty([])):
        ...
        (k, bf) = self.loadpk(bstft+self.suf+self.get_snap(redshift)+self.ext, box)
        kbins = self.Getkbins()
    ...
    def plot_error_sdss(self, Sim, bstft, box, derivs,
                        params, qarams=np.array([]), zzz=np.array([]), colour="",
                        ylabel="", title="", ymax=0, ymin=0, legend=False):
    ...
    def plot_error_all(self, Sim, bstft, box, derivs, params,
                       zzz=np.array([]), colour="", ylabel="",
                       title="", ymax=0, ymin=0, legend=False):
        ...
        for zz in zzz:
            ...
            if colour != "":
                line = np.append(line,
                                 plt.semilogx(self.Getkbins(), err, color=colour))
            else:
...
class flux_pow(power_spec):
    ...
    """ A class written to store the various methods related to
    calculating of the flux derivatives and plotting of the flux power spectra"""
    ...
    def __init__(self, Snaps=("Snapshot_000", "Snapshot_001", "Snapshot_002", "Snapshot_003",
                 ...
                 sdsskbins=np.array([0.00141, 0.00178, 0.00224, 0.00282, 0.00355,
                                     0.00447, 0.00562, 0.00708, 0.00891, 0.01122,
                                     0.01413, 0.01778]),
                 knotpos=np.array([0.07, 0.15, 0.475, 0.75, 1.19, 1.89, 4, 25]),
                 om=0.266, H0=0.71, box=60.0, kmax=4.0,
...
class MATTER_pow(power_spec):
    ...
    def __init__(self, Snaps=("Snapshot_000", "Snapshot_001", "Snapshot_002", "Snapshot_003",
                 ...
                 knotpos=np.array([0.07, 0.15, 0.475, 0.75, 1.19, 1.89, 4, 25]),
                 om=0.266, ob=0.0449, H0=0.71, box=60.0,
                 base="/home/spb41/Lyman-alpha/MinParametricRecon/runs/",
                 suf="MATTER-power/", ext=".0", matpre="PK-by-"):
        power_spec.__init__(self, Snaps, Zz, sdsskbins, knotpos, om, H0, box, base, suf, ext)
        self.ob = ob
        self.pre = matpre

    def plot_z(self, Knot, redshift, title="Relative MATTER Power",
               ylabel=r"$\mathrm{P}(k, p)\, /\, \mathrm{P}(k, p_0)$",legend=True):
        power_spec.plot_z(self, Knot, redshift, title, ylabel,legend=legend)
    ...
    def plot_power(self, path, redshift, camb_filename="",colour="black"):
        ...
        plt.loglog(k_g, Pk_g*(1+sigma*(2.0/sqrt_err+1.0/samp_err)), linestyle="-.", color=colour)
        plt.loglog(k_g, Pk_g*(1-sigma*(2.0/sqrt_err+1.0/samp_err)), linestyle="-.", color=colour)
        if camb_filename != "":
...
class flux_pdf(power_spec):
    ...
    def __init__(self, Snaps=("Snapshot_006", "Snapshot_007", "Snapshot_008",
                 ...
                 base="/home/spb41/Lyman-alpha/MinParametricRecon/runs/",
                 suf="flux-pdf/", ext="_flux_pdf.txt", ):
        power_spec.__init__(self, Snaps, Zz, sdsskbins, knotpos, om, H0, box, base, suf, ext)
    ...
    def plot_power(self, path, redshift, camb_filename="", colour = "black"):
        """ Plot absolute power spectrum, not relative"""
        (k, Pdf) = self.loadpk(path+self.suf+self.pre+\
                               self.get_snap(redshift)+self.ext, self.box)
...
#        return inarr
    ...
    def calc_z(self, redshift, s_KNOT, kbins):
        ...
        for k in np.arange(0, nk):
            dPF = self.flux_deriv(PowerFluxes[:, k]/PFp0[k], pdifs)[0]
            d2PF = self.flux_deriv(PowerFluxes[:, k]/PFp0[k], pdifs)[1]
            results[k] = d2PF
            results[nk+k] = dPF
        return results
    ...
    def plot_z(self, Knot, redshift, title="", ylabel="",legend=True):
        ...
        #Load best-fit
        (simk, bfkp) = self.loadpk(Knot.bstft+self.suf+self.pre+\
                                   self.get_snap(redshift)+self.ext, self.bfbox)
        #Setup figure plot.
        ind = wheref(self.Zz, redshift)
        plt.figure(ind[0][0])
        plt.clf()
        ...
        legname = np.array([])
        for sim in Knot.names:
            Pk = self.loadpk(sim+self.suf+\
                                  self.pre+self.get_snap(redshift)+self.ext, self.box)[1]
            line = np.append(line, plt.semilogy(simk, Pk/bfkp, linestyle="-", linewidth=1.5))
            legname = np.append(legname, sim)
        plt.legend(line, legname)
        return
    ...
    def GetFlat(self, dirr, si=0.0):
        ...
        z = 2.07
        PF_a = self.loadpk(dirr+self.suf+"Snapshot_011"+self.ext, self.box)[1]
        PF_b = self.loadpk(dirr+self.suf+"Snapshot_010"+self.ext, self.box)[1]
        PF1 = (z-2.0)*5*(PF_b-PF_a)+PF_a
        z = 2.52
        PF_a = self.loadpk(dirr+self.suf+"Snapshot_009"+self.ext, self.box)[1]
        PF_b = self.loadpk(dirr+self.suf+"Snapshot_008"+self.ext, self.box)[1]
        PF2 = (z-2.4)*5*(PF_b-PF_a)+PF_a
        z = 2.94
        PF_a = self.loadpk(dirr+self.suf+"Snapshot_007"+self.ext, self.box)[1]
        PF_b = self.loadpk(dirr+self.suf+"Snapshot_006"+self.ext, self.box)[1]
...
class flux_interp:
    ...
    def GetPFSingleKnot(self, i, redshift, param, qaram=np.array([])):
        ...
        if np.size(self.p0[i]) > 1:
            ...
            if np.size(self.q0) > 0 and np.size(self.q0[i]) > 1:
                ...
                total = self.derivs[i, 0:kbins, ind]*dp**2 + \
                self.derivs[i, kbins:2*kbins, ind]*dp +  self.derivs[i, 2*kbins:3*kbins, ind]* \
                dq**2+self.derivs[i, 3*kbins:4*kbins, ind]*dq
    ...
    def get_mv_tables(self, j, flux):
        ...
        Zz = self.Zz[ind]
        nz_ = np.size(Zz)
        nk_ = np.size(self.kbins)
        # ???
        tmp = self.derivs[j, 0:, ind][0].T
        nco = np.shape(self.derivs)[1]/nk_
        nsk = np.size(flux.get_sdss_bins(Zz[0]))
        deriv_r = np.zeros([nco*nsk, nz_])
        for q_ in np.arange(nco):
            for i in np.arange(0, nz_):
                deriv_r[q_*nsk:(q_+1)*nsk, i] = rebin(tmp[q_*nk_:(q_+1)*nk_, i],
                                                      self.kbins, flux.get_sdss_bins(Zz[i]))
        return deriv_r

    @classmethod
    def get_pdf_tables(cls, pdf, s_knot):
        """Get the pdf tables"""
        #Flux pdf has 3 redshift bins, 20 k bins.
        _nk = 21
        _nz = 3
        #Redshift bins are at 2.07, 2.52 and 2.94
        _pz = np.array([2.07, 2.52, 2.94])
        _fd = np.zeros([2*_nk, _nz])
        #Call flux_deriv_const_z for each redshift
        #This gets them in increasing redshift.
        for i in np.arange(0, _nz):
            _fd[:, i] = pdf.calc_z(_pz[i], s_knot)
        return _fd


class MatteoFluxInterp(flux_interp):
    ...
    def __init__(self, file, p0):
        ...
        self.p0 = np.array([p0])
        self.kbins = np.array([0.00141, 0.00178, 0.00224, 0.00282,
                               0.00355, 0.00447, 0.00562, 0.00708,
                               0.00891, 0.01122, 0.01413, 0.01778])

    def dpf(self, _z, _p):
        """Easier wrapper"""
        return self.GetPFSingleKnot(0, _z, _p)


    def tpf(self, _z, _t):
        """Use this function if the variable in question is tau"""
#        tbf = np.array([0.896000, 0.756733, 0.657800, 0.600400, 0.513000,
#                        0.42900, 0.379867, 0.3285330, 0.2714000, 0.2192, 0.178000])
        tbf = 0.0023*(1+self.Zz)**3.65

        ind = wheref(self.Zz, _z)
        return (_t/tbf[ind]-1)*(self.derivs[0][ind, :, 0])+\
            (self.derivs[0][ind, :, 1])*(_t/tbf[ind]-1)**2
...
if __name__ == '__main__':
    ...
    AA_KNOT = knot(("AA0.54/", "AA0.74/", "AA1.14/", "AA1.34/"),
                   (0.54, 0.74, 1.14, 1.34), 0.94, "boxcorr400/", 120)
...
#Check knot
    ...
    G_KNOT = knot(("bf2z/", "bf2b/", "bf2c/", "bf2d/", "bf2T15/", "bf2T35/", "bf2T45/", "bf2a/"),
                  (BF2ZG, BF2BG, BF2CG, BF2DG, T15G, T35G, T45G, BF2AG), BF2G, "bf2/", 60,
                  (BF2ZT, BF2BT, BF2CT, BF2DT, T15T, T35T, T45T, BF2AT), BF2G)
