...
class power_spec:
    ...
    def GetFlat(self, dirc, si=0.0):
        ...
        for i in np.arange(0, np.size(self.Snaps) - 1):
            ...
            (k, Pk) = self.loadpk(dirc + self.Snaps[i] + self.ext, self.box)
...
class matter_pow(power_spec):
    ...
    def plot_z(
        ...
        power_spec.plot_z(self, Knot, redshift, title, ylabel)
...
class flux_pdf(power_spec):
    ...
    def loadpk(self, path, box):
        flux_pdfo = np.loadtxt(self.base + path)
        return (flux_pdfo[:, 0], flux_pdfo[:, 1])
    ...
    def GetFlat(self, dirc, si=0.0):
        ...
        np.savetxt(sys.stdout, PDF.T, fmt="%1.8f", delimiter=' ')
        return (PF1, PF2, PF3)
