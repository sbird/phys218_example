...
#Corr is /sqrt(self.H0)...
    ...
    def GetFlat(self,onedir, si=0.0):
        ...
        for i in np.arange(0,np.size(self.Snaps)-1):
            ...
            (k,Pk)=self.loadpk(onedir+self.Snaps[i]+self.ext, self.box)
...
class matter_pow(power_spec):
    ...
    def plot_z(self,Knot,redshift,title="Relative Matter Power",ylabel=r"$\mathrm{P}(k,p)\,/\,\mathrm{P}(k,p_0)$",legend=False):
        power_spec.plot_z(self,Knot,redshift,title,ylabel,legend)
    ...
    def plot_power_2(self,path, redshift,colour="black", camb_filename=""):
        """ Plot absolute power spectrum, not relative"""
...
class flux_pdf(power_spec):
    ...
    def smooth_rebin(self, inarr,k,newk):
        """Wrapper around smooth_rebin to allow us not to do it."""
        (inarr,_,_)=self.smooth_rebin(inarr,k,newk)
        return inarr
    ...
    def plot_z(self,Knot,redshift,title="",ylabel="",legend=False):
        """ Plot comparisons between a bunch of sims on one graph 
        plot_z(Redshift, Sims to use ( eg, A1.14). 
        Note this will clear current figures."""
        #Load best-fit
    ...
    def GetFlat(self,onedir, si=0.0):
        ...
        z=2.07
        (_,PF_a)=self.loadpk(onedir+self.suf+"snapshot_011"+self.ext, self.box)
        (_,PF_b)=self.loadpk(onedir+self.suf+"snapshot_010"+self.ext, self.box)
        PF1=(z-2.0)*5*(PF_b-PF_a)+PF_a
        z=2.52
        (_,PF_a)=self.loadpk(onedir+self.suf+"snapshot_009"+self.ext, self.box)
        (_,PF_b)=self.loadpk(onedir+self.suf+"snapshot_008"+self.ext, self.box)
        PF2=(z-2.4)*5*(PF_b-PF_a)+PF_a
        z=2.94
        (_,PF_a)=self.loadpk(onedir+self.suf+"snapshot_007"+self.ext, self.box)
        (_,PF_b)=self.loadpk(onedir+self.suf+"snapshot_006"+self.ext, self.box)
        PF3=(z-2.8)*5*(PF_b-PF_a)+PF_a
        PDF = np.array([PF1,PF2,PF3])
        #np.savetxt(sys.stdout,PDF.T("%1.8f","%1.8f","%1.8f"))
        np.savetxt(sys.stdout,np.transpose(PDF)("%1.8f","%1.8f","%1.8f"))
        return (PF1, PF2, PF3)
...
class matteo_flux_interp(flux_interp):
    ...
    def __init__(self, file, p0):
        ...
        super(matteo_flux_interp, self).__init__(file)
        self.file = file
