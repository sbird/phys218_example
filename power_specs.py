...
class DataError(Exception):
    def __init__(self, value):
        ...
        super(DataError,self).__init__(value)
        self.value = value
    def __str__(self):
        return repr(self.value)

# """ Smooth and rebin data points """
# Function removed 11/20/19 because "Smooth" module has yet to be written 
...
#                 relP=smooth_rebin(relP,onek)
    ...
    def GetFlat(self,dir_flat, si=0.0): #changed 'dir to 'dir_flat' because 'dir' is a bad variable name
        ...
        for i in np.arange(0,np.size(self.Snaps)-1):
            ...
            (k,Pk)=self.loadpk(dir_flat+self.Snaps[i]+self.ext, self.box)
    ...
    def GetDiff(self,dir1,dir2,onebox,twobox):
        ...
        for z in self.Zz:
            ...
            relP=relP/rebin(oneP, onek, onek[onei][twoi])
            onek=onek[onei][twoi]
            #relP=smooth_rebin(relP,onek,self.GetSDSSkbins(z)) # we can add this back if we can load the 'smooth' module1 11/20/19 
            i=i+1
            diff[-i,:]=relP
    ...
    def flux_deriv(self, PFdif, pdif, qdif=np.array([])):
        ...
        #(derivs, residues,rank, sing)=np.linalg.lstsq(mat, PFdif) #maybe we need residues rank and sing later. 11/20/19
        derivs = np.linalg.lstsq(mat, PFdif)[0] # we only need derivatives. 11/20/19
        return derivs
    ...
    def Getkbins(self): #copy pasted from flux_pow
        """Get the kbins to interpolate onto"""
        flux_pow.Getkbins(self.sdsskbins)
        #return self.sdsskbins
    def loadpk(self, path, box): #copy pasted from flux_pow
        ...
        # PF=flux_power[1:,1]/scale
        # return (k, PF)
        flux_pow.loadpk(path, box) # ASSUMING SAME FUNCTION AS "flux_pow"
...
class flux_pdf(power_spec):
    ...
    def __init__(self, Snaps=("snapshot_006","snapshot_007","snapshot_008","snapshot_009","snapshot_010","snapshot_011"),
        ...
        knotpos=np.array([]), om=0.266,H0=0.71,box=48.0,#ob=0.0449, H0=0.71,box=48.0,
        base="/home/spb41/Lyman-alpha/MinParametricRecon/runs/",suf="flux-pdf/", ext="_flux_pdf.txt",):
        power_spec.__init__(self, Snaps,Zz,sdsskbins,knotpos, om, H0,box,base,suf,ext)

    def loadpk(self, path, box):
        flux_pdf_local = np.loadtxt(self.base+path)
        return(flux_pdf_local[:,0], flux_pdf_local[:,1])
    ...
    def plot_power(self,path, redshift,colour="black"): # (self,path, redshift,colour="black"):
        """ Plot absolute power spectrum, not relative"""
    ...
    def calc_z(self, redshift,s_knot, kbins): #added 'kbins' 11-21-2019 #calc_z(self, redshift,s_knot, kbins):
        """ Calculate the flux derivatives for a single redshift
    ...
    def plot_z(self,Knot,redshift,title="",ylabel="",legend=True): #plot_z(self,Knot,redshift,title="",ylabel="", legend=True):
        """ Plot comparisons between a bunch of sims on one graph 
        plot_z(Redshift, Sims to use ( eg, A1.14). 
        Note this will clear current figures."""
        #Load best-fit
        ...
        for sim in Knot.names:
            #(k,Pk)=self.loadpk(sim+self.suf+self.pre+self.GetSnap(redshift)+self.ext,self.box)  
            Pk=self.loadpk(sim+self.suf+self.pre+self.GetSnap(redshift)+self.ext,self.box)[1]
    ...
    def GetFlat(self,dir_flat, si=0.0): #GetFlat(self,dir_flat, si=0.0):
        """Get a power spectrum in the flat format we use for inputting some cosmomc tables"""
        #Pk_sdss=np.empty([11, 12])
        #For z=2.07 we need to average snap_011 and snap_010
        z=2.07
        #(k,PF_a)=self.loadpk(dir+self.suf+"snapshot_011"+self.ext, self.box)
        PF_a=self.loadpk(dir_flat+self.suf+"snapshot_011"+self.ext, self.box)[1]
        #(k,PF_b)=self.loadpk(dir_gf+self.suf+"snapshot_010"+self.ext, self.box)
        PF_b=self.loadpk(dir_flat+self.suf+"snapshot_010"+self.ext, self.box)[1]
        PF1=(z-2.0)*5*(PF_b-PF_a)+PF_a
        z=2.52
        #(k,PF_a)=self.loadpk(dir_flat+self.suf+"snapshot_009"+self.ext, self.box)
        #(k,PF_b)=self.loadpk(dir_flat+self.suf+"snapshot_008"+self.ext, self.box)
        PF_a=self.loadpk(dir_flat+self.suf+"snapshot_009"+self.ext, self.box)[1]
        PF_b=self.loadpk(dir_flat+self.suf+"snapshot_008"+self.ext, self.box)[1]
        
        PF2=(z-2.4)*5*(PF_b-PF_a)+PF_a
        z=2.94
        #(k,PF_a)=self.loadpk(dir_flat+self.suf+"snapshot_007"+self.ext, self.box)
        #(k,PF_b)=self.loadpk(dir_flat+self.suf+"snapshot_006"+self.ext, self.box)
        PF_a=self.loadpk(dir_flat+self.suf+"snapshot_007"+self.ext, self.box)[1]
        PF_b=self.loadpk(dir_flat+self.suf+"snapshot_006"+self.ext, self.box)[1]
...
class flux_interp:
    ...
    def __init__(self,flux_local, Knots):
        if np.shape(Knots) == ():
            Knots=(Knots,)
        Knots=np.array(Knots)
        self.kbins=flux_local.Getkbins()
        tmp=np.fliplr(flux_local.calc_all(Knots[0],self.kbins))
        ...
        if np.size(Knots[0].p0) > 1:
            ...
            for i in np.arange(1,np.size(Knots)):
                self.derivs[i]=np.fliplr(flux_local.calc_all(Knots[i],self.kbins))
        ...
        self.Zz=np.flipud(flux_local.Zz)
    ...
    def Get_MV_Tables(self,j,flux_local):
        ...
        nsk=np.size(flux_local.GetSDSSkbins(Zz[0]))
        deriv_r=np.zeros([nco*nsk,nz])
        for q in np.arange(nco):
            for i in np.arange(0,nz):
                deriv_r[q*nsk:(q+1)*nsk,i]=rebin(tmp[q*nk:(q+1)*nk,i],self.kbins,flux_local.GetSDSSkbins(Zz[i]))
        return deriv_r
...
class matteo_flux_interp(flux_interp):
    """Load pre-calculated tables in the style Matteo uses, Load a table in Matteo's format."""
    def __init__(self, file, p0):
        super(matteo_flux_interp,self).__init__(file, p0)
