...
def save_figure(path):
    ...
    if path == "":
        return  ret
    if bk in ('TkAgg', 'Agg', 'GTKAgg'):
        path = path + ".png"
        ret = plt.savefig(path)
    elif bk in ('PDF', 'pdf'):
        path = path + ".pdf"
        ret = plt.savefig(path)
    elif bk in ('PS', 'ps'):
...
def corr_table(table, dvecs, table_name):
    ...
    np.savetxt(pkd + table_name, new,\
    ("%1.3g", "%1.3g", "%1.3g", "%1.3g", "%1.3g", "%1.3g",\
     "%1.3g", "%1.3g", "%1.3g", "%1.3g", "%1.3g"))
    return new

class PowerSpec:
    """ A class to be derived from by flux and matter PowerSpec\
    ...
    def __init__(self, Snaps=("snapshot_000", "snapshot_001", "snapshot_002", "snapshot_003",\
                 ...
                 knotpos=np.array([0.07, 0.15, 0.475, 0.75, 1.19, 1.89, 4, 25]), om=0.267,\
                 H0=0.71, box=60.0, base="/home/spb41/Lyman-alpha/MinParametricRecon/runs/",\
                 suf="/", ext=".txt"):
        if len(Snaps) != np.size(Zz):
            raise DataError("There are " + str(len(Snaps)) + " snapshots, but " +\
                            str(np.size(Zz)) + "redshifts given.")
        self.Snaps = Snaps
        self.Zz = Zz
    ...
    def plot_z(self, Knot, redshift, title="", ylabel="", legend=True):
        ...
        # Load best-fit
        (simk, BFPk) = self.loadpk(Knot.bstft + self.suf + self.pre +\
                                   self.GetSnap(redshift) + self.ext, self.bfbox)
        # Setup figure plot.
        ind = wheref(self.Zz, redshift)
        plt.figure(ind[0][0])
        plt.clf()
        ...
        for sim in Knot.names:
            (k, Pk) = self.loadpk(sim + self.suf + self.pre +\
                                  self.GetSnap(redshift) + self.ext, self.box)
    ...
    def plot_compare_two_all(self, onedir, twodir, zzz=np.array([]),\
        ...
        legname = np.array([])
        for z in zzz:
            line = np.append(line, self.plot_compare_two(onedir + self.pre +\
                                                         self.GetSnap(z) + self.ext, self.bfbox,\
                                                         twodir + self.pre + self.GetSnap(z) +\
                                                         self.ext, self.box, colour))
            legname = np.append(legname, "z=" + str(z))
        if title == "":
        ...
        for i in np.arange(0, np.size(self.Snaps) - 1):
            ...
            for j in np.arange(1, np.size(sdss)):
                sicorr[j] = 1 + a ** 2 + 2 * a * (math.sin(2271 * kmids[j + 1]) -\
                                                  math.sin(2271 * kmids[j])) /\
                            (kmids[j + 1] - kmids[j]) / 2271
            sdss = self.GetSDSSkbins(self.Zz[i])
            Pk_sdss[-i - 1, :] = rebin(Pk, k, sdss) * scale * sicorr
        return Pk_sdss

    def GetDiff(self, dir1, dir2, onebox, twobox):
        """Gets the difference between two power spectra, in arbitrary\
         directories, with arbitrary box sizes"""
    ...
    def plot_error_sdss(self, Sim, bstft, box, derivs, params, qarams=np.array([]),\
                        zzz=np.array([]), colour="", ylabel="", title="",\
                        ymax=0, ymin=0, legend=False):
    ...
    def plot_error_all(self, Sim, bstft, box, derivs, params, zzz=np.array([]), colour="",\
...
class flux_pow(PowerSpec):
    ...
    def __init__(self, bf="best-fit/", Snaps=("snapshot_000", "snapshot_001",\
        ...
        self.bf = bf
        k_bf = self.loadpk(self.bf + self.suf + "snapshot_000" + self.ext, self.bfbox)[0]
    ...
    def plot_z(self, Knot, redshift, title="Relative Flux Power",\
               ...
               legend=True):
        PowerSpec.plot_z(self, Knot, redshift, title, ylabel, legend)
        if legend:
...
class matter_pow(PowerSpec):
    ...
    def __init__(self, Snaps=("snapshot_000", "snapshot_001",\
                 ...
                 Zz=np.array([4.2, 4.0, 3.8, 3.6, 3.4, 3.2, 3.0, 2.8, 2.6, 2.4, 2.2, 2.0]), \
                 sdsskbins=np.array([0.00141, 0.00178, 0.00224,\
                                     0.00282, 0.00355, 0.00447, 0.00562, 0.00708,\
    ...
    def plot_z(self, Knot, redshift, title="Relative Matter Power",\
        ...
        PowerSpec.plot_z(self, Knot, redshift, title, ylabel)
...
class flux_pdf(PowerSpec):
    ...
    def calc_z(self, redshift, s_knot, kbins):
        ...
        for i in np.arange(0, np.size(s_knot.names)):
            (k, uPower[i, :]) = self.loadpk(s_knot.names[i] +\
                                            self.suf + usnap + self.ext, s_knot.bfbox)
        (k, lPFp0) = self.loadpk(s_knot.bstft + self.suf + lsnap + self.ext, s_knot.bfbox)
        lPower = np.zeros((npvals, np.size(k)))
        for i in np.arange(0, np.size(s_knot.names)):
            (k, lPower[i, :]) = self.loadpk(s_knot.names[i] +\
                                            self.suf + lsnap + self.ext, s_knot.bfbox)
        ...
        for k in np.arange(0, nk):
            (dPF, d2PF) = self.flux_deriv(PowerFluxes[:, k] / PFp0[k], pdifs)[0, 1]
            results[k] = d2PF
            results[nk + k] = dPF
        return results
    ...
    def plot_z(self, Knot, redshift, title="", ylabel="",legend=True):
        ...
        legname = np.array([])
        for sim in Knot.names:
            Pk = self.loadpk(sim + self.suf + self.pre +\
                                  self.GetSnap(redshift) + self.ext, self.box)[1]
    ...
    def GetFlat(self, dir,si=0.0):
        ...
        z = 2.07
        PF_a = self.loadpk(dir + self.suf + "snapshot_011" + self.ext, self.box)[1]
        PF_b = self.loadpk(dir + self.suf + "snapshot_010" + self.ext, self.box)[1]
        PF1 = (z - 2.0) * 5 * (PF_b - PF_a) + PF_a
        z = 2.52
        PF_a = self.loadpk(dir + self.suf + "snapshot_009" + self.ext, self.box)[1]
        PF_b = self.loadpk(dir + self.suf + "snapshot_008" + self.ext, self.box)[1]
        PF2 = (z - 2.4) * 5 * (PF_b - PF_a) + PF_a
        z = 2.94
        PF_a = self.loadpk(dir + self.suf + "snapshot_007" + self.ext, self.box)[1]
        PF_b = self.loadpk(dir + self.suf + "snapshot_006" + self.ext, self.box)[1]
...
class knot:
    ...
    def __init__(self, sims, pvals, p0, bstft, bfbox, qvals=np.array([]), q0=0):
        ...
        if np.size(sims) != np.size(pvals) and np.size(self.pvals[:, 0]) != np.size(sims):
            raise DataError(str(np.size(pvals)) + " pvals given, but " +\
                            str(np.size(sims)) + " sims.")
...
class flux_interp:
        ...
        if np.size(self.p0[i]) > 1:
            ...
            if np.size(self.q0) > 0 and np.size(self.q0[i]) > 1:
                ...
                total = self.derivs[i, 0:kbins, ind] * dp ** 2 +\
                        self.derivs[i, kbins:2 * kbins, ind] * dp +\
                 self.derivs[i, 2 * kbins:3 * kbins, ind] * dq ** 2 +\
                        self.derivs[i, 3 * kbins:4 * kbins, ind] * dq
    ...
    def Get_MV_Tables(self, j, flux):
        ...
        for q in np.arange(nco):
            for i in np.arange(0, nz):
                deriv_r[q * nsk:(q + 1) * nsk, i] = rebin(tmp[q * nk:(q + 1) * nk, i], self.kbins,
                                                          flux.GetSDSSkbins(Zz[i]))
        return deriv_r
...
class matteo_flux_interp(flux_interp):
    ...
    def __init__(self, file, p0):
        ...
        self.p0 = np.array([p0])
        self.kbins = np.array([0.00141, 0.00178, 0.00224, 0.00282, 0.00355, 0.00447, 0.00562,\
    ...
    def tpf(self, z, t):
        ...
        #tbf = np.array([0.896000,0.756733,0.657800,0.600400,0.513000,0.42900,0.379867,\
        # 0.3285330,0.2714000,0.2192,0.178000])
...
if __name__ == '__main__':
    ...
    G_knot = knot(("bf2z/", "bf2b/", "bf2c/", "bf2d/", "bf2T15/", "bf2T35/", "bf2T45/", "bf2a/"),
                  ...
                  (bf2zt, bf2bt, bf2ct, bf2dt, T15t, T35t, T45t, bf2at), bf2t)
